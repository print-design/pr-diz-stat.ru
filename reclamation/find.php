...
<form class="form-inline ml-auto mr-3<?=$find_class ?>" method="get" id="find-form" action="select_calculation_id.php">
    <div class="input-group input-group-sm<?=$group_class ?>" id="find-group">
        ...
        <?= filter_input(INPUT_GET, "find") ?>
            &nbsp;&nbsp;
            <a href="select_calculation_id.php<?= (filter_input(INPUT_GET, 'status') === null ? "" : "?status=". filter_input(INPUT_GET, 'status')) ?>"><i class="fas fa-times" style="color: white;"></i></a>
        </div>
    </div>
</form>
<?php
